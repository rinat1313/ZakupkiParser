...
func (w *Worker) tick(ctx context.Context) {
	...
	outcome := w.Pipeline.Resolve(ctx, item.RegNumber, csvSite)
	w.logItem(ctx, job, item, "info", "источник: %s — %s", outcome.SourceUsed, outcome.Message)

	if outcome.FailedAnalyze || outcome.Result == nil {
		msg := outcome.Message
		if msg == "" {
		...
		stubPayload, _ := json.Marshal(map[string]any{
			"csv_source":  csvSite,
			"source_used": outcome.SourceUsed,
			"failed":      true,
			"message":     msg,
		})
		tender, _, upErr := w.Store.UpsertTender(ctx, db.TenderUpsertInput{
	...
	if strings.HasPrefix(outcome.SourceUsed, "adapter:") {
		storeSite = csvSite
	}
	payload := injectMeta(res.Payload, csvSite, outcome.SourceUsed)
	tender, _, err := w.Store.UpsertTender(ctx, db.TenderUpsertInput{
...
func (w *Worker) logItem(ctx context.Context, job *db.IngestJob, item *db.IngestItem, level, format string, args ...any) {
...
func StartIngest(ctx context.Context, store *db.Store, categorySlug, categoryTitle, sourceName string, items []struct{ Reg, Site string }) (*db.IngestJob, error) {
	var cat *db.Category
	var err error
	if categorySlug != "" {
		cat, err = store.GetCategoryBySlug(ctx, categorySlug)
		if err != nil {
			return nil, err
		}
	} else {
		cat, err = store.CreateCategory(ctx, categoryTitle, "")
		if err != nil {
			return nil, err
		}
	}
	return store.CreateIngestJob(ctx, cat.ID, sourceName, items)
}
